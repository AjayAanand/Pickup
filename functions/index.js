...
exports.updatedb = functions.https.onRequest(async (request, response) => {
    const langmain = require("./language-en.json");
    const usersdata = await admin.database().ref("users").once("value");
    ...
    for (const key in users) {
      admin
        ...
        .update({
          usertype:
            users[key] && users[key].usertype
              ? users[key].usertype === "rider"
                ? "customer"
                : users[key].usertype
              : "customer",
          createdAt:
            users[key] && users[key].createdAt
              ? new Date(users[key].createdAt).getTime()
              : new Date().getTime(),
        });
    }
    const laguagedata = await admin.database().ref("languages").once("value");
    const languages = laguagedata.val();
    for (const key in languages) {
      const langitem = languages[key].keyValuePairs;
      const merged = Object.assign({}, langmain, langitem);
      admin
        .database()
        .ref("languages/" + key + "/keyValuePairs")
        .set(merged);
    }
