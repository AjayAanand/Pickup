...
const createFullStructure = (app, db, auth, storage, config) => {
    return {
        ...
        driverDocsTsl: (uid) => stRef(storage, `users/${uid}/tsl`),
        driverDocsP_endoursmenFront: (uid) => stRef(storage, `users/${uid}/p_endoursmenImageFront`),
        driverDocsP_endoursmenBack: (uid) => stRef(storage, `users/${uid}/p_endoursmenImageBack`),
        driverDocsCof: (uid) => stRef(storage, `users/${uid}/cof`),
        driverDocsVehicelRegistration: (uid) => stRef(storage, `users/${uid}/vehicelRegistration`),
        driverDocsVehicelInsurance: (uid) => stRef(storage, `users/${uid}/vehicelInsurance`),
        driverDocsAirportAccessCard: (uid) => stRef(storage, `users/${uid}/airportAccessCard`),         
